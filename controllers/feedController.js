...
export const fetchPets = async (req, res) => {
	try {
		...
		if (cityStreet && cityStreet.trim() !== '') {
			...
			if (parts.length >= 2) {
				...
				where.push(
					`(pets.city ILIKE ${pCity} AND pets.street ILIKE ${pStreet})`
				);
    ...
    `;
		...
		const unit =
			(ageUnit || 'years').toLowerCase() === 'months' ? 'months' : 'years';
		const aFrom = parseIntOrNull(ageFrom);
		const aTo = parseIntOrNull(ageTo);

		const toMonths = (val) =>
			val === null ? null : unit === 'months' ? val : val * 12;
...
export const fetchPetById = async (req, res) => {
	try {
		...
		const sql = `
			SELECT pets.*, users.phone, users.first_name, users.surname, pets.id
			FROM ${tableName} AS pets
			JOIN users_data.users AS users
			ON pets.owner = users.email
			WHERE pets.id = $1
		...
		console.error('FETCH PET BY ID ERROR:', error.message);
		res.status(500).json({ error: 'Database error' });
	}
};
